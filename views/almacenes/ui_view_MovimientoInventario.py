...
class ViewMovimientoInventario(QDialog, Messages):
    ...
    def registarMovimiento(self):
        ...
        if (productSelected and almacen1Selected and almacen2Selected and cantidad > 0):
            if (self._type == "cargo"):
                response = self._controller.register_load(producto, origen, destino, cantidad)
            else:
                response = self._controller.register_unload(producto, origen, destino, cantidad)
            return response
